...
const Profile = () => {
  ...
  const renderDashboard = () => (
    <div className="space-y-6">
      {/* Top Row - Focus, Weekly Activity, AI Mentor, Achievements */}
            ...
            <div className="p-2 rounded-lg bg-muted/50 border">
              ...
              <p className="text-xs font-medium">
                {completedInCareer < careerRelatedSlugs.length 
                  ? `Complete your ${career?.name || 'career'} courses`
                  : 'Explore advanced topics next'}
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              {achievements.length > 0 ? (
                achievements.slice(0, 2).map((achievement) => {
                  ...
                  return (
                    <div 
                      key={achievement.id} 
                      className={`flex items-center gap-2 p-2 rounded-lg ${bgColor} border ${borderColor}`}
                    >
                ...
                <>
                  ...
                  {completedInCareer > 0 && (
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-primary/10 border border-primary/20">
                      ...
                      <p className="text-xs font-medium">{completedInCareer} Course{completedInCareer !== 1 ? 's' : ''}</p>
                    </div>
                  )}
                  {currentStreak < 7 && completedInCareer === 0 && (
                    <div className="text-center py-2 text-muted-foreground">
                      <Award className="h-6 w-6 mx-auto mb-1 opacity-30" />
                      <p className="text-xs">Start learning!</p>
                    </div>
                  )}
                </>
              )}
      ...
      {/* Dashboard Header + Career Readiness */}
      <div className="space-y-6">
