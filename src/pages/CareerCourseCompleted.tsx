 ...
 * 
 * This page is rendered ONLY for courses accessed within the Career Board shell.
 * It celebrates course completion, provides certification, and guides the learner
 * toward next steps within their career path.
 * 
 ...
 * - It does NOT show ads or guest banners (premium experience)
...
const CareerCourseCompleted = () => {
  ...
  // Prefer route param for stable URLs
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // If no skills from careers, use some defaults
        if (skills.length === 0) {
        ...
        // Fetch recommended next course (from same career, within careerCourses)
        if (careerCourses.length > 0) {
          // Find next course in career that isn't the current one
          const currentIndex = careerCourses.findIndex(c => c.slug === courseSlug);
          const nextInCareer = careerCourses.find((c, index) => index > currentIndex);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
        {/* Back link - goes to course page within career shell */}
        <Link
        ...
        {/* 5. NEXT STEPS - Show next course within career path */}
        <section className="mb-8">
          <NextStepsCard
            recommendedCourse={nextCourse ? {
              ...nextCourse,
              // Override the navigation to stay within career shell
