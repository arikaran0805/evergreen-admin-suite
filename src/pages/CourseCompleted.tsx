/**
 * CourseCompleted - Dedicated Course Completion Page
 * 
 * Route: /course/:courseId/completed
 * 
 * Celebrates course completion, provides certification,
 * and guides the learner toward next steps.
 * 
 * This page appears ONLY after the user completes the final lesson.
 * Certificate download is exclusive to this page.
 */
...
const CourseCompleted = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (careerCoursesData && careerCoursesData.length > 0) {
          const { data: nextCourseData } = await supabase
            ...
            `)
            .eq("career_id", careerCoursesData[0].career_id)
            .neq("course_id", courseId)
            .is("deleted_at", null)
            .limit(1);

          if (nextCourseData?.[0]?.courses) {
            setNextCourse(nextCourseData[0].courses as unknown as CourseData);
          }
        }
    ...
    if (!authLoading) {
      if (!user) {
        navigate("/auth", { state: { from: `/course/${courseId}/completed` } });
      } else {
        fetchData();
      }
    }
  }, [courseId, user, authLoading, navigate, toast]);
    ...
    <Layout>
      <div className="min-h-screen bg-background">
        <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
          ...
          <section className="mb-8">
            <NextStepsCard
              recommendedCourse={nextCourse || undefined}
            />
          </section>
        </div>
      </div>
