...
export interface PredictOutputMapping {
  id: string;
...
export function usePredictOutputMappingsBySkill(
  skillId: string | undefined,
  subTopicIds: string[]
) {
  return useQuery({
    queryKey: ["predict-output-mappings-by-skill", skillId, subTopicIds],
    queryFn: async () => {
      if (!skillId || subTopicIds.length === 0) return [];
      const { data, error } = await supabase
        .from("predict_output_mappings")
        .select("*, predict_output_problems(*)")
...
export function useCreatePredictOutputMapping() {
  ...
  return useMutation({
    mutationFn: async (mapping: {
      ...
      const { data, error } = await supabase
        .from("predict_output_mappings")
        .insert({
          predict_output_problem_id: mapping.predict_output_problem_id,
...
export function useDeletePredictOutputMapping() {
  const queryClient = useQueryClient();
  return useMutation({
